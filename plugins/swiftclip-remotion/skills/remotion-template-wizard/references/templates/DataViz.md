...
| --- | --- | --- |
| Section title | `"System Load"` | edit text node directly |
| Subtitle | `"Real-time usage"` | edit text node directly |
| Progress bar fill | `progress * 100 + "%"` | change `progress` interpolation range |
| Bar color | `#0066cc` | edit inline style |
| Background | `#f5f5f7` | edit `AbsoluteFill` style |

## Visual style

Full-screen layout with frosted glass card. Title top-left, horizontal progress/metric bars stacked below. Bars grow from 0 on entrance. 20s gives time for multiple metric reveals.

## Use cases

KPI reports, system metrics, analytics recaps, business review videos, data storytelling.
