...
| --- | --- | --- |
| Prompt text | `"Siri, create a Dashboard layout..."` | edit `promptText` constant |
| Orb color / style | petal-based orb with spring entrance | edit `getPetalStyle` calls |
| Morph target size | `720×88` (chat input box) | edit `boxWidth` / `boxHeight` interpolation end values |
| Generated card content | UI layout shown after morph | edit the card result section |
| Typing speed | `frame * 0.8` chars/frame | edit multiplier |

## Visual style

Sequence: AI orb appears with spring → morphs into a chat input box → prompt text types out → UI result card slides in. Apple aesthetic throughout.

## Use cases

AI product demos, Siri / copilot feature showcases, generative UI walkthroughs, SaaS AI feature promos.
